...
class Slider_Controller extends CI_Controller 
{
	 public function __construct()
	 {
			...
			$this->load->model('admin/slider/Slider_Model');
	...
	public function index()
	{
		...
		$total_row = $this->Slider_Model->record_count();
		...
		{ 
			   ...
			   $result['records'] = $this->Slider_Model->getall($config["per_page"],$page,0);
	...
	public function uploadimg()
	{
		 ...
		 if ($this->session->userdata('is_user_login')) 
		{
			...
			if ($_FILES["file_image"]["size"] <2097152) 
			{
				...
				{
					$filename ='slider_'.rand(10000, 990000) . '_' . time() . '.' . $file_extension;				
		...
		if ($this->session->userdata('is_user_login')) 
		{
			...
			else 
			{
			 ...
			 $arr = array(
			  ...
			  $result = $this->Slider_Model->update($arr);
			   if ($result== TRUE) 
			   {
				  $data['success']=$result['success'];
			   }	 
