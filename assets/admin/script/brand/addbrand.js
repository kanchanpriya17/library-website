...
{   
	...
	$("#txt_title").focus();
		...
		$("#btn_save").click(function()
		{
			    ...
			    if($("#event_name").val()=='')
				{
				   $("#event_name").parent().find("#div_err").remove();							   
		           $("#event_name").parent().append('<div id="div_err" class="alert alert-danger" role="alert">This Field Can not be left blank</div>');	
				}
				/*if($("#event_date").val()=='')
				{
				   $("#event_date").find("#div_err").remove();							   
		           $("#event_date").append('<div id="div_err" class="alert alert-danger" role="alert">This Field Can not be left blank</div>');	
				}*/
				else if($("#event_time").val()=='')
				{
				   $("#event_time").parent().find("#div_err").remove();							   
		           $("#event_time").parent().append('<div id="div_err" class="alert alert-danger" role="alert">This Field Can not be left blank</div>');	
				}
				else
				{
			      ...
			      $.ajax
						({
							url:''+baseurl+'brandsave',
							type: "POST", 
							data:{event_name:$("#event_name").val(),event_time:$("#event_time").val(),event_description:CKEDITOR.instances.editor.getData(),event_image_name:$("#hid_upload").val(),tag:'save'},
							cache: false,							
							success: function(data)  		
							{
								var data	=	$.parseJSON(data);	
