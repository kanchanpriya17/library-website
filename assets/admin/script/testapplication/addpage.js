				...
				else
				{
			      ...
			      $.ajax
						  ({
							...
							type: "POST", 
							data:{title:$("#txt_title").val(),sub_title:$("#txt_sub_title").val(),description:CKEDITOR.instances.editor.getData(),image_name:$("#hid_upload").val(),parent_page_id:$("#slct_page").val(),tag:'save',header_image:$("#hid_upload1").val()},
							cache: false,							
							success: function(data)  		
							{
								var data	=	$.parseJSON(data);	
...
function load_page()
{
	...
	$("#slct_page").append('<option value="0">Select</option>');
	 $.ajax
	  ({
