	...
	load_category();
				...
				else
				{
					   ...
					   /* var files = !!this.files ? this.files : [];
						...
						({
							...
							type: "POST", 
							data: new FormData(),	
							data:  new FormData(document.getElementById("frm_gallery")),
							contentType: false,       		
							cache: false,					
							processData:false,  							
		...
		$("#btn_save").click(function()
		{
				...
				else
				{
			      ...
			      $.ajax
						({
							url:''+baseurl+'gallerysave1',
							type: "POST", 
							data:{title:$("#txt_title").val(),sub_title:$("#txt_sub_title").val(),description:CKEDITOR.instances.editor.getData(),image_name:$("#hid_upload").val(),album_id:$("#slct_album").val(),tag:'save'},
							cache: false,							
							success: function(data)  		
							{
								var data	=	$.parseJSON(data);	
...
function load_category()
{
	 ...
	 $("#slct_album").append("<option value='0'>Select</option>");
	 $.ajax
	  ({
		...
		{        var data	=	$.parseJSON(data);	
				$.each(data, function(k,v) 
				{
					 if(k=='records')
					 {
						  for(i in v)
						 {
							  $("#slct_album").append('<option value='+v[i]['event_id']+'>'+v[i]['event_name']+'</option>');
						 }	 
					 }
				});
		}
